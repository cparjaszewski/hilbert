...
function plot_extended_2d(fun,args)
...
%A=3.1;T1=0.4;fun = @(x) (1 ./ (A - x - 1i.*T1));args=0.01:0.003:8;
...
% Lorentzian:
% fun = @(x)(2.*Area/pi).*(Width./(4.*((x-Centers).^2)+Width.^2));
f=@(x)imag(fun(x)); 
...
if (getGK==true)
  ...
  elseif (plotAbsValues==true)
      plot(args, abs(valuesGK), 'Color','green');    
  else
...
if (getCC==true)
  ...
  elseif (plotAbsValues==true)
      plot(args, abs(valuesCC), 'Color','cyan');    
  else
      plot(args, valuesCC, 'Color','cyan');
  end;
end;
if (getTR==true)
  ...
  elseif (plotAbsValues==true)
      plot(args, abs(valuesTR), 'Color','magenta');    
  else
      plot(args, valuesTR, 'Color','magenta');
  end;
end;
%plot(args, imag(fun(args)),  'Color','red');
if (plotAbsValues==true)
   plot(args, abs(real(fun(args))), 'Color','yellow');    
else
   plot(args, real(fun(args)), 'Color','yellow');
end;
...
function sum = cc_sum(g, y, a, b)
  ...
  sum = 1/2 * (c(1) * m(1)); % first coefficient is divided by two
  for i=2:1:n
      sum = sum + c(i) * m(i);
  end;
  sum = sum + 1/2 * (c(n+1) * m(n+1)); % last coefficient is divided by two
  sum = (b-a)/2 * sum;
