...
\begin{subequations} \label{eq:htran_r3interp}
  ...
  \begin{equation}   \label{eq:hr3inp_next}
    \widehat{R_{k}} = \frac { - {R_{k - 1}} + 9\,{R_{k}} + 9\,{R_{k + 1}} - {R_{k + 2}}}{16} \quad \mbox{ for } \, k = 2\,\ldots\,{N - 2}
  \end{equation}
...
the results in the Figure (\ref{eq:htran_lin}). In this and in the following chapters we would like the error function
...
The results for such set of parameters are shown on the Figure (\ref{fig:htran_pnp_2d}).
...
similar to the original ones, but we have received a huge relative error as presented in the Figure (\ref{fig:htran_pnp_3derr}).
...
For the linear susceptibility we have used the model from the Chapter (\ref{chap:problem_quantum}): 
...
Results are presented in the Figure (\ref{fig:nc_quantum2}). We can see a very large relative error reaching up to the 100\%. We can also
see that this model seems to be invalid. This will be checked with another methods in next chapters.
...
The first step suggested by the authors is to remove the singularity from the integral (equation \ref{eq:hcc_singularity}) :
...
now - is that the well-chosen, finite interval for integration stated in (\ref{eq:hcc_interval}) or an integral with a period few times
bigger.
...
Results are presented in the Figure (\ref{fig:four_qp2}). We can see, that the first model could be valid, while the second model seems to
...
As shown of the Figure (\ref{fig:oa_plot}) the open-aperture transmittance can be described with the approximation equation:
